...
abstract class FoormManagerBase
{
    ...
    public function setFoormBasicData($formName)
    {
...
//        $this->setFixedConstraintsToParams($params);
        $params['is_queue'] = $this->isQueue;
        if (!array_key_exists('is_api', $params)) {
            $params['is_api'] = $this->checkIfIsApi();
        }
        if (!array_key_exists('from_action', $params)) {
            $params['from_action'] = false;
        }
        $this->params = $params;
    }
    ...
    protected function setFoorm()
    {
        ...
        $totalInput = $this->buildInput();

        $input = $this->setInputForForm($totalInput);
